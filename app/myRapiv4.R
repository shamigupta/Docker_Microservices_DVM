...
function(myquerry) {
  ...
  #print(myquerry)
  conn = dbConnect(driver,"jdbc:rs:dv://192.168.66.2:1200;DatabaseType=DVS; user=ashissa, password=73e613cc896030ac3e  ", user="ashissa", password="73e613cc896030ac3e")
...
function(myquerry) {
  ...
  #print(myquerry)
  conn = conn = dbConnect(driver,"jdbc:db2://192.168.66.2:5035/DALLASB", user="ashissa", password="A9SHISAH", ":memory:")
...
function(myquerry) {
  ...
  .jaddClassPath( "/srv/jdbc/dv-jdbc-3.1.jar" )
  driver <- JDBC("com.rs.jdbc.dv.DvDriver",Sys.glob("/srv/jdbc/*.jar"))
...
function(myquerry) {
  .jaddClassPath( "/srv/jdbc/db2jcc.jar" )
  driver <- JDBC("com.ibm.db2.jcc.DB2Driver",Sys.glob("/srv/jdbc/*.jar"))
  #print(myquerry)
  conn = conn = dbConnect(driver,"jdbc:db2://192.168.66.2:5040/DALLASC", user="ashissa", password="A9SHISAH", ":memory:")
  if (tolower(str_extract(myquerry, '\\w*')) != "select") {
    result = dbSendUpdate(conn, myquerry)
    dbCommit(conn)
  }
...
function(l1, mycurrency) {
  ...
  outdata <- data.frame()
  for (i in 1:NROW(l1)) {
    urlname <- paste("http://192.168.66.2:9080/catalogManager/items/",l1[i,1],sep="")
...
function(x, y) {
  ...
  #Validate Account
  if (!processing_error) {
    urlname <- paste("http://192.168.66.2:9080/jkebanking/accno/",account_num,sep="")
  ...
  if (!processing_error) {
    ...
    res <- PUT(paste("http://192.168.66.2:9080/jkebanking/accno/",account_num,sep=""),body=pc_json,encode ="json")
    appData <- content(res)
  ...
  if (!processing_error) {
    for (i in 1:NROW(order_input)) {
      ...
      while (loop_order) {
        ...
        res <- POST("http://192.168.66.2:9080/catalogManager/orders",body = pc_json,encode = "json")
        appData <- content(res)
        ...
        if (ordermessage !=  "ORDER SUCCESSFULLY PLACED") {
          urlname <- paste("http://192.168.66.2:9080/catalogManager/items/",order_input[i,1],sep="")
          itemrefdata <- fromJSON(urlname)
          d1 <- as.data.frame(itemrefdata[[1]][[2]][[1]],stringsAsFactors=F)
